...
convDLV_dm <- function(yearOrRegion){
  ...
  # Parameters
  lst <- list(lowerV = c(-2,-2,.8,.8,0.001,-5,-5),
  ...
  # Function for optimizing lnviDM2
  optimizator <- function(i){
    inits1  <- lst$inits1
    temp1 <- constrOptim(inits1, function(p) -lnviDM2(p,pPanel[,i]), NULL, ui = const_mat[,-ncol(const_mat)], const_mat[,ncol(const_mat)])
    if(temp1$convergence!=0){out <- temp1} else {
      inits2 <- lst$inits2
      temp2  <- constrOptim(inits2, function(p) -lnviDM2(p,pPanel[,i]), NULL, ui = const_mat[,-ncol(const_mat)], const_mat[,ncol(const_mat)])
      if(temp2$convergence != 0) {out <- temp2} else {
        inits3 <- lst$inits3
        temp3  <- constrOptim(inits3, function(p) -lnviDM2(p,pPanel[,i]), NULL, ui = const_mat[,-ncol(const_mat)], const_mat[,ncol(const_mat)])
