...
convDLV <- function(yearOrRegion){
  ...
  lowerV <- c(-2,-2,.8,.8,0.001,-50,-50,-50)
  upperV <- c(2,2,.999,.999,50,50,50,50)
  inits  <- c(.75,  1.2,  0.90,  0.99,  0.01, 0.1,0.1,0.1)
  ...
  res1 <- simplify2array(lapply(from[1]:to[1], function(n){
    cat(paste0(n,"\n"))
    temp <- tryCatch(optim(inits, function(p) -lnviDM2(p,pPanel[,n]),lower = lowerV,
  ...
  res2 <- simplify2array(lapply(from[2]:to[2], function(n){
    temp <- tryCatch(optim(inits, function(p) -lnviDM2(p,pPanel[,n]),lower = lowerV,
                           upper = upperV,method="L-BFGS-B"), error=function(e){NA})
    temp <- tryCatch(c(temp$par,temp$value),error=function(e){rep(NA,7)}); return(temp)}))
  ...
  res3 <- simplify2array(lapply(from[3]:to[3], function(n){
    temp <- tryCatch(optim(inits, function(p) -lnviDM2(p,pPanel[,n]),lower = lowerV,
                           upper = upperV,method="L-BFGS-B"), error=function(e){NA})
    temp <- tryCatch(c(temp$par,temp$value),error=function(e){rep(NA,7)}); return(temp)}))
  ...
  res4 <- simplify2array(lapply(from[4]:to[4], function(n){
    temp <- tryCatch(optim(inits, function(p) -lnviDM2(p,pPanel[,n]),lower = lowerV,
                           upper = upperV,method="L-BFGS-B"), error=function(e){NA})
    temp <- tryCatch(c(temp$par,temp$value),error=function(e){rep(NA,7)}); return(temp)}))
  ...
  res5 <- simplify2array(lapply(from[5]:to[5], function(n){
    temp <- tryCatch(optim(inits, function(p) -lnviDM2(p,pPanel[,n]),lower = lowerV,
                           upper = upperV,method="L-BFGS-B"), error=function(e){NA})
    temp <- tryCatch(c(temp$par,temp$value),error=function(e){rep(NA,7)}); return(temp)}))
